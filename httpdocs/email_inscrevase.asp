...
Session.Timeout = 60 'valor em minutos
Session.LCID = 1046 
%>
...
Dim cmd, formacao, tipo_pagamento
	...
	If Err.number <> 0 then 
    	if instr(Err.description, "@cod_turma") > 0 then 	 
	    	...
	    	Response.AddHeader "Refresh", "5;URL=http://www.thepilatesfisiofitness.com.br/" 
	    	response.end
		end if
  	End If
		...
		<div class="missed-deals-area" id="pl-showdeal"><!-- &nbsp; --></div> 
                ...
                <div align="left">
                  <select name="parcela" id="parcela" class="form-control input-lg" style="margin-bottom:20px">
                    ...
                    <%for i = 0 to 9
					if i = 0 then%>
                    <option value="<%=(i+1)%>"><%=(i+1)&"x de "&formatcurrency(CDbl(preco_curso)/1)&" À vista"%></option>
                    <%else%>
                    <option value="<%=(i+1)%>"><%=(i+1)&"x de "&formatcurrency(CDbl(preco_curso)/(i+1))&" Sem juros"%></option>
                    <%end if
					next%>
                  </select>
                </div>
            ...
            <%'Final de Pagamento com Cartão
				...
				'response.write(session("Sessao_Inscricao"))
				%>
				
            	<!--#include file="itau-transicao.asp"-->
