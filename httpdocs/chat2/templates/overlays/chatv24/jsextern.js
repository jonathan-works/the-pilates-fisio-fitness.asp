    ...
    if(lz_overlay_chat != null)
    {
        ...
        else
        {
            ...
            if(OverlayChatWidgetV2.__CurrentOperator != null && _typingText != null && !lz_operator_typing)
            {
                ...
                thtml = thtml.replace("name=<!--ename-->&ebg=<!--bgce-->","operator=" + encodeURIComponent(OverlayChatWidgetV2.__CurrentOperator.Id));
...
function lz_kb_set_url(_searchFor){
    ...
    var url = lz_poll_server + 'knowledgebase.php?tops=MQ__&no-logo=MQ__&etc=' + lz_global_base64_url_encode(lz_color_secondary) + '&ptl=' + LiveZillaData.Language  + "&ckf=" + lz_kb_root;
    if(_searchFor!='')
...
function lz_chat_set_members(_groupId,_show,_list){
    ...
    for(var k in _list)
    {
        var avbg = (!_list[k].op) ? 'name=' + _list[k].name : 'operator=' + encodeURIComponent(_list[k].id);
...
function lz_chat_poll_parameters(){
    ...
    {
        params += "&operator=" + encodeURIComponent(OverlayChatWidgetV2.__TargetOperatorId);
    }

    if(OverlayChatWidgetV2.__TargetGroupId != null)
    {
        params += "&eg=" + encodeURIComponent(OverlayChatWidgetV2.__TargetGroupId);
    }
...
function lz_chat_decline_request(_id,_operator,_stateChange,_result,_closeWindow){
...
OverlayChatWidgetV2.SetHost = function(_systemId,_bot,_chatId,_groupId,_groupName,_userid,_lang,_image,_fullname){
    if(lz_overlay_chat != null)
    {
        OverlayChatWidgetV2.__CurrentChatId = lz_global_base64_decode(_chatId);
...
OverlayChatWidgetV2.UpdateWMUI = function(){
    ...
    for(var i=lz_ovlel.length-1;i>=0;i--)
    {
        ...
        else
        {
            ...
            OverlayChatWidgetV2.UpdateRootIcon(cln);
        }
...
OverlayChatWidgetV2.UpdateChatFunctions = function(){
    ...
    if(functions != null)
    {
        var inChat = OverlayChatWidgetV2.__CurrentChatId != 0 && (lz_chat_status == 2 || (lz_chat_status == 1 && OverlayChatWidgetV2.__ModeBot && OverlayChatWidgetV2.BotFeedback)) && !OverlayChatWidgetV2.__PublicGroupChat.length;
    ...
    if(OverlayChatWidgetV2.OfflineMessageMode == 1)
    {
        if(lz_overlay_chat.m_FullScreenMode || _click)
        {
            if(!OverlayChatWidgetV2.OfflineMessageNewWindow)
                ...
                OverlayChatWidgetV2.OpenWindow(OverlayChatWidgetV2.OfflineMessageHTTP,true);
        }
        return true;
    }
    return false;
...
OverlayChatWidgetV2.SetMode = function(_type,_click){
    ...
    else
    {
        ...
        {
            _type = '';
            OverlayChatWidgetV2.DeclineChatInvite(_click);
        }
        else if(_type == 'wm')
        {
            OverlayChatWidgetV2.DeclineChatInvite(_click);
...
OverlayChatWidgetV2.DeclineChatInvite = function(_click){
    if(_click && document.getElementById("lz_chat_invite_id") != null && lz_chat_status == 0)
    {
        lz_chat_decline_request(lz_request_active=document.getElementById("lz_chat_invite_id").value,false,false,false);
    }
};
...
OverlayChatWidgetV2.UpdateUI = function(){
    ...
    if(!OverlayChatWidgetV2.__ModeChangeDetails)
    {
        if(lz_session.OVLWMSElem == "chat" && document.getElementById("lz_chat_invite_id") != null)
            OverlayChatWidgetV2.__FormVisible = false;
        else if(lz_session.OVLWMSElem == "chat" && (OverlayChatWidgetV2.__ChatStarted || OverlayChatWidgetV2.__InitChat || OverlayChatWidgetV2.__Connecting))
            OverlayChatWidgetV2.__FormVisible = false;
...
OverlayChatWidgetV2.UpdateRootIcon = function(cln){

    if(lz_session.OVLWMState == 1)
        cln.getElementsByTagName('path')[0].setAttribute("d", lz_get_icon_path('close'));
    else
    {
        cln.getElementsByTagName('path')[0].setAttribute("d", lz_get_icon_path('comments'));
        if(OverlayChatWidgetV2.__ModeSingle)
        {
...
OverlayChatWidgetV2.LeaveMessage = function(_click,_groupId){
    ...
    OverlayChatWidgetV2.SetMode('ticket',_click);
};
