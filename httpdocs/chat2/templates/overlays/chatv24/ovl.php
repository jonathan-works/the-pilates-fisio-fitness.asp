...
if(!empty(VisitorMonitoring::$Visitor->Browsers[0]->DesiredChatGroup) && isset(Server::$Groups[VisitorMonitoring::$Visitor->Browsers[0]->DesiredChatGroup]) && !(IS_FILTERED && !FILTER_ALLOW_CHATS && !FILTER_ALLOW_TICKETS))
{
    ...
    if(Visitor::$PollCount == 1)
    {
        ...
        VisitorMonitoring::$Response .= "lz_tracking_add_overlay_chat_v2('".base64_encode($OVERLAY->ChatHTML)."','".base64_encode(Server::$Configuration->File["gl_site_name"])."',".$ovlw.",".$ovlh.",".$ml.",".$mt.",".$mr.",".$mb.",'".Communication::ReadParameter("ovlp",22)."',".$br.");";
    ...
    $OVERLAY->ProcessIdleExit(VisitorMonitoring::$Visitor->Browsers[0]);
    ...
    if($OVERLAY->IsChatAvailable && !Visitor::$OpenChatExternal && !empty(VisitorMonitoring::$Visitor->Browsers[1]->ChatRequest) && Server::$Operators[VisitorMonitoring::$Visitor->Browsers[1]->ChatRequest->SenderSystemId]->IsExternal(Server::$Groups,null,null))
    {
        if(!isset($_GET["hinv"]) && !VisitorMonitoring::$Visitor->Browsers[1]->ChatRequest->Closed && !VisitorMonitoring::$Visitor->Browsers[1]->ChatRequest->Accepted && VisitorMonitoring::$Visitor->Browsers[0]->Status == 0)
        {
            ...
            if(!VisitorMonitoring::$Visitor->Browsers[1]->ChatRequest->Displayed)
            {
                ...
                VisitorMonitoring::$Visitor->AddFunctionCall("OverlayChatWidgetV2.ExitBotChat(true,false,4);",false);
                VisitorMonitoring::$Visitor->AddFunctionCall("OverlayChatWidgetV2.UpdateUI();",false);

                //VisitorMonitoring::$Visitor->AddFunctionCall("OverlayChatWidgetV2.__TargetGroupId = '" . VisitorMonitoring::$Visitor->Browsers[1]->ChatRequest->SenderGroupId . "';",false);
                //VisitorMonitoring::$Visitor->AddFunctionCall("OverlayChatWidgetV2.SetUIGroup('".VisitorMonitoring::$Visitor->Browsers[1]->ChatRequest->SenderGroupId."');",false);
    ...
    if(!$OVERLAY->IsChatAvailable)
        VisitorMonitoring::$Visitor->AddFunctionCall("lz_chat_set_connecting(false,'".VisitorMonitoring::$Visitor->Browsers[0]->SystemId."',null,0);",false);
    else
