...
else
{
	...
	else
	{
        ...
        if($isClassicElement)
            $isChatAvailable = operatorsAvailable(0,$parameters["exclude"],$parameters["include_group"],$parameters["include_user"],false);
        ...
        if(!empty($_GET["fbpos"]) && !empty($_GET["fbw"]))
        {
            ...
            if(!(!empty($_GET["fboo"]) && !$isChatAvailable))
		...
		if(!empty($_GET["cboo"]) && !$isChatAvailable)
		...
		{
            if(!empty($_GET["ovlc"]))
                VisitorMonitoring::$Response .= "LiveZillaTracking.Callback(" . VisitorMonitoring::GetPollFrequency(VisitorMonitoring::$Visitor->IsInChat(false)) . ");";
            else
                VisitorMonitoring::$Response .= "LiveZillaTracking.Callback(" . VisitorMonitoring::GetPollFrequency(false) . ");";

            if($isClassicElement)
                VisitorMonitoring::$Response .= "LiveZillaTracking.UpdateTextLinks(".To::BoolString($isChatAvailable).");";
		}
	}
}
?>
