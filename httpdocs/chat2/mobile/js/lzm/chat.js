...
function chatInputEnterPressed() {
    ...
    else if (!KnowledgebaseUI.QuickSearchReady && edContent.indexOf('/') == 0)
    {
        if (lzm_chatDisplay.qrdAutoSearch == 1)
        {
            ...
            KnowledgebaseUI.ChatKBAutoSearch(true);
        }
    }
    else
    {
...
function chatInputTyping(e) {
    ...
    $('#chat-progress').css({'bottom': (137 + ChatEditorClass.ExpandChatInputOffset) + 'px'});
}
...
function uploadFile(fileType, parentId, rank, _ticketReplyWindowId, sendToChat) {
    ...
    if (typeof file != 'undefined')
    {
        ...
        CommunicationEngine.uploadFile(file, fileType, parentId, rank, _ticketReplyWindowId, sendToChat);
    }
    else
...
function personalChatLink(){
    ...
    if(lzm_commonPermissions.permissions.personal_chat_link != 1)
    {
        showNoPermissionMessage();
        return;
    }

    var link = DataEngine.serverProtocol + DataEngine.serverUrl.replace(':80/','/').replace(':443/','/') + '/chat.php?operator=' + encodeURIComponent(DataEngine.myUserId);
...
function initLinkGenerator(e){
    ...
    if (DataEngine.operators.getOperator(lzm_chatDisplay.myId).level == 1) {
        $.getScript('js/lzm/classes/LinkGeneratorClass.js', function( data, textStatus, jqxhr ) {
            ...
            if(TaskBarManager.WindowExists('link-generator-dialog'))
            {
...
function showLogs(){
    ...
    if(lzm_commonPermissions.permissions.view_logs != 1)
    {
        showNoPermissionMessage();
        return;
    }
...
function switchTicketListPresentation(ticketFetchTime, counter, ticketId) {
    ...
    var loadingHtml;
    if (counter == 0 && lzm_commonPermissions.permissions.tickets != 0)
    {
        ...
        $('#ticket-list-loading').css({position: 'absolute', left: left, top:0,bottom:0,right:0,'background-color': '#fff', 'z-index': 1000, opacity: 1});
    }
...
function setTicketOperator(ticketId, operatorId) {
    ...
    selTickets.each(function()
    {
        ...
        if (myTicket != null)
        {
            var ticketGroup = myTicket.gr;
            var ticketStatus = (typeof myTicket.editor != 'undefined' && myTicket.editor != false) ? myTicket.editor.st : 0;
            ChatTicketClass.InitUploadTicketDetails(myTicket, myTicket.t, ticketStatus, ticketGroup, operatorId, myTicket.l, '', '', '', '', '');
        }
    });
    ChatTicketClass.UploadTicketDetails();
}
...
function setTicketGroup(ticketId, groupId) {
    ...
    selTickets.each(function()
    {
        ...
        if (myTicket != null)
        {
            ...
            var ticketStatus = (typeof myTicket.editor != 'undefined' && myTicket.editor != false) ? myTicket.editor.st : 0;
            ChatTicketClass.InitUploadTicketDetails(myTicket, myTicket.t, ticketStatus, groupId, ticketEditor, myTicket.l, '', '', '', '', '');
        }
    });
    ChatTicketClass.UploadTicketDetails();
}
...
$(document).ready(function () {
    ...
    CommonUIClass.BlockNavigationHidden = $(window).width() < 700;
