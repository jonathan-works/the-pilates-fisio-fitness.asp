...
ChatSettingsClass.prototype.createSettingsHtml = function(){
    ...
    if(!IFManager.IsMobileOS)
    {
        ...
        {
            notificationSettings += '<table id="notification-settings-table" style="width:auto">';
    ...
    var dis = (IFManager.IsDesktopApp() && IFManager.DeviceInterface.hasModule && IFManager.DeviceInterface.hasModule('devtools',1)) ? '' : 'ui-disabled';

    generalSettings += '<div class="top-space-double">' + lzm_inputControls.createButton('app-console',dis,'IFManager.IFToggleDevTools()','Developer Tools','<i class="fa fa-code"></i>','force-text',{padding:'6px 10px'},'',30,'e');
    generalSettings += lzm_inputControls.createButton('clear-storage','','LocalConfiguration.Reset()','Clear Local Data','<i class="fa fa-trash"></i>','force-text',{padding:'6px 10px','margin-left':'6px'},'',30,'e');
...
ChatSettingsClass.prototype.createUserManagement = function(){
    ...
    var bodyString = '<iframe id="user-management-iframe" onload="ChatSettingsClass.__UserManagementLoaded();" src="admin.php?acid=' + acid + '&type=user_management&lang=' + lz_global_base64_url_encode(lzm_t.language) + '"></iframe>';
...
LocalConfiguration.TicketSearchSettings = '1111111011101';
        ...
        else if (table == 'archive' && columnArray instanceof Array)
        {
            if (columnArray instanceof Array && columnArray.length == 0)
            {
                newColumnArray = [
                    ...
                    {index: 20, cid: 'ticketid', title: 'Ticket ID', display: 1}
                ];
                LocalConfiguration.AddCustomBlock(newColumnArray);
            }
        }
