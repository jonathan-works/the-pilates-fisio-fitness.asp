...
ChatPollServerClass.prototype.PollServerResource = function (_objects, _action) {
    ...
    if (!thisClass.pollIsActive)
    {
        ...
        if(_action=='set')
        {
            ...
            {
                // NEW
                dataObject['p_process_resources_vj_0'] = resource.l;
                dataObject['p_process_resources_vk_0'] = resource.p;
                dataObject['p_process_resources_vl_0'] = resource.f;
                dataObject['p_process_resources_vm_0'] = resource.s;
                dataObject['p_process_resources_vn_0'] = resource.ba;
            }
        ...
        for (key in tickets)
        {
            ...
            else if (type == 'new-ticket')
            {
                ...
                if (_chat.cid != '')
                {
                    ...
                    ticketDataObject['p_ta_' + count + '_vd_1'] = _chat.cid;
                }
                ...
                {
                    count++;
                    ticketDataObject['p_ta_' + count + '_vc'] = 'LinkFeedback';
                    ticketDataObject['p_ta_' + count + '_vd_0'] = tempId;
                    ticketDataObject['p_ta_' + count + '_vd_1'] = _feedback.i;
                }
                */
            }
...
ChatPollServerClass.prototype.uploadFile = function(file, fileType, parentId, rank, _ticketReplyWindowId, sendToChat, _toKb, _toTicketComment) {
    var thisClass = this;
    ...
    else
    {
        setTimeout(function()
        {
            thisClass.uploadFile(file, fileType, parentId, rank, _ticketReplyWindowId, sendToChat);
        }, 500);
    }
};
            ...
            if (xmlIsLiveZillaXml)
            {
                ...
                if (DataEngine.myId != '')
                {
                    ...
                    if (thisClass.qrdRequestTime == 0)
                    {
                        ...
                        var qrdAutoSearch = lzm_commonStorage.loadValue('kb_auto_search_' + DataEngine.myId);
