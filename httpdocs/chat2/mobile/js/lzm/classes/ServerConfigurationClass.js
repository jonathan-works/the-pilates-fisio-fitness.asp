...
ServerConfigurationClass.prototype.resetInputFields = function() {
    $('#sc-inputs-list').parent().html(this.m_Config.GetCustomForm('Inputs',LzmCustomInputs.GetDefaultInputList()));
};
    ...
    this.m_Settings = [
        {name:'Chats', title: tid('chats'), icon:'comments', groups: [
            ...
            {name: 'WaitingTime', title: tid('waiting_time'), controls: [
                ...
                {name:'gl_acco', standard:true, type:'bool', value: 0, title: tidc('auto_close_operator'), top:'double'},
                {name:'gl_ccoa', standard:true, type:'int', value: 180, title: '', class:'',  left:'single', titleright: tid('seconds'), bottom:'single', top:'half'},
                {name:'gl_accc', standard:true, type:'bool', value: 0, title: tidc('auto_close_customer'), top:'double'},
                {name:'gl_ccca', standard:true, type:'int', value: 180, title: '', class:'',  left:'single', titleright: tid('seconds'), bottom:'single', top:'half'}
            ]},
        ...
        {name:'Tickets', title: tid('tickets'), icon:'envelope',groups: [
            {name: 'General', title: tid('general'), controls: [
                ...
                {name:'gl_tprs_label', type:'label', title: tidc('ticket_after_reply_status'), top:'single'},
...
ServerConfiguration.prototype.GetCustomForm = function(formType, obj) {
    ...
    if(formType== 'EmailAccounts'){
        ...
        contentHtml += '<table id="ticket-sub-statuses" class="visible-list-table lzm-unselectable alternating-rows-table"><thead><tr><th colspan="2">'+tid('status')+'</th></tr></thead><tbody>' + ss_html.replace(/<!--type-->/g,'channels');
        ...
        contentHtml += '<table id="ticket-sub-channels" class="visible-list-table lzm-unselectable alternating-rows-table"><thead><tr><th colspan="2">'+tid('channel')+'</th></tr></thead><tbody>' + sc_html.replace(/<!--type-->/g,'statuses');
        contentHtml += '</tbody></table>';
        ...
        contentHtml = '<table class="spaced" style="max-width:1000px;"><tr><td colspan="3"><fieldset class="lzm-fieldset-full" style="margin-top:-10px;height:360px;"><legend>'+tid('general')+'</legend>';
