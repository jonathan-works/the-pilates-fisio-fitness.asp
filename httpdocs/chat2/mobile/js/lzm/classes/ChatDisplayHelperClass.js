...
ChatDisplayHelperClass.prototype.maximizeDialogWindow = function(dialogId) {
    ...
    if (winObj.TypeId == 'chat-window')
    {
        ...
        KnowledgebaseUI.ChatKBAutoSearch(false);
...
ChatDisplayHelperClass.prototype.RenderMainMenuPanel = function() {
    ...
    panelHtml += '<div id="main-menu-panel-status" class="main-menu-panel-tool lzm-unselectable" onclick="showUserStatusMenu(event); UIRenderer.resizeMenuPanels();" style="background-image: url(\'' + statusIcon + '\');">&nbsp;</div>';
    panelHtml += '<div id="main-menu-panel-settings" class="main-menu-panel-tool lzm-unselectable" onclick="showUserSettingsMenu(event); UIRenderer.resizeMenuPanels();"><div id="main-menu-panel-settings-text">' + lzm_chatDisplay.myName + '</div></div>';
    panelHtml += '<span id="main-menu-panel-tools-configuration" onclick="initServerConfiguration(event);" class="main-menu-panel-opt-tools main-menu-panel-tool" title="'+tid('server_conf')+'"><i class="fa fa-cogs fa-lg"></i></span>' +
        ...
        '<span id="main-menu-panel-tools-logs" onclick="showLogs();" class="main-menu-panel-opt-tools main-menu-panel-tool" title="Logs"><i class="fa fa-exclamation-circle fa-lg '+((Client.Logs.length>0 && lzm_commonPermissions.permissions.view_logs == 1) ? 'icon-blue' : '')+'"></i></span>' +
        '<span id="main-menu-panel-tools-wish" style="border-right:0;" onclick="openLink(\'http://wishlistmobile.livezilla.net/\');" class="main-menu-panel-opt-tools main-menu-panel-tool" title="'+tid('make_wish')+'"><i class="fa fa-plus fa-lg"></i></span>';
...
ChatDisplayHelperClass.prototype.createGeotrackingFootline = function() {
    ...
    var gtFootlineHtml = '<span style="float: left;">' +
        ...
        lzm_inputControls.createButton('map-visitor-info', 'map-button' + disabledClass, 'SelectView(\'external\');ChatVisitorClass.ShowVisitorInformation(\'' + visitorId + '\');', '', '<i class="fa fa-info"></i>', 'lr', {'margin-left': '3px','margin-right': '3px'}, t('Show visitor information')) +
