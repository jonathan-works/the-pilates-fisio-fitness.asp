	...
	while(isset($_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vc"]))
	{
		...
		else if($type == "AddTicketEditorReply")
		{
            ...
            if($Ticket->Channel >= 6 && !empty($Ticket->ChannelId))
            {
                ...
                if($Ticket->Channel == 7 && $twchannel->StreamType == 1)
                    $Ticket->Messages[0]->Text = $twchannel->AddScreenName($Ticket->Messages[0]->Text,$_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vd_2"]);
            ...
            $Ticket->SendOperatorReply($Ticket->Messages[0]->Id,$_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vd_2"],$_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vd_6"],$_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vd_ecc"],$_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vd_ebcc"]);

            if(!empty($_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_newst"]))
            {
                $Ticket->LoadStatus();
                $Ticket->Editor->Status = intval($_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_newst"]);
                $Ticket->Editor->Save();
            }
		...
		else if($type == "CreateTicket")
		{
            ...
            if(!empty($_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vd_4"]))
            {
                ...
                $Ticket->Messages[0]->HTML = DBManager::DecodeField($email->BodyHTML);
                ...
                $Ticket->Messages[0]->EmailCC = $email->CC;
            }
            ...
            if(isset($_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vyy"]))
                Chat::SetTicketId($_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vyy"],"",$Ticket->Id);

            if(isset($_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vyz"]))
            {
                $Ticket->Messages[0]->SenderUserId =
                $Ticket->SenderUserId = $_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $count . "_vyz"];
            }
