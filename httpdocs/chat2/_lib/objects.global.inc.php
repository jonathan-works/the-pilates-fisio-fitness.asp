...
class Communication
{
    static function DownloadSocialMedia($_fromAPI=false)
    {
        ...
        if(is_array(Server::$Groups) && !empty(Server::$Configuration->Database["gl_sm"]))
        {
                        ...
                        foreach($newTickets as $hash => $ticket)
                        {
                            ...
                            foreach($ticket->Messages as $index => $message)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        // ticket channel id has changed, message belongs to old ticket
                                        Logging::DebugLog("Ticket Message exists: " . $message->TicketId . " / " . $existMessTicketId);
            ...
            if($temporaryPruning)
            {
                Logging::DebugLog("Temporary prune");
                            ...
                            if(!empty($newmails) && is_array($newmails))
                                foreach($newmails as $temail)
                                {
                                    ...
                                    foreach($temail->Attachments as $attid => $attdata)
                                    {
                                        ...
                                        KnowledgeBase::Process("SYSTEM", $attid, $attdata[0], 3, $attdata[1], 0, 100, 0,$tags);
    ...
    static function ReadParameter($_key, $_fallBack=null,$_b64encoded=true)
    {
        ...
        $types["int"] = array("pc","fbpos","fbt","ovlts","h","po","cid","tc","echsp","echp","ovlsx","ovlsy","ovlsb","deactr","fbshb","fbshx","fbshy","fbml","fbmt","fbmr","fbmb","fbw","fbh","ovlp","ovlw","ovlh","ovlbr","ovlml","ecw","ech","cid","ecslw","ecmb","ecfo");
...
class Post extends BaseObject
{
	...
	function Save($_mTime=0)
	{
		...
		DBManager::Execute(true, "INSERT INTO `" . DB_PREFIX . DATABASE_POSTS . "` (`id`,`chat_id`,`time`,`micro`,`sender`,`receiver`,`receiver_group`,`receiver_original`,`text`,`translation`,`translation_iso`,`received`,`persistent`,`repost`,`sender_name`,`browser_id`,`api_obj`) VALUES ('" . DBManager::RealEscape($this->Id) . "','" . DBManager::RealEscape($this->ChatId) . "'," . DBManager::RealEscape($this->Created) . "," . DBManager::RealEscape($_mTime[0]) . ",'" . DBManager::RealEscape($this->Sender) . "','" . DBManager::RealEscape($this->Receiver) . "','" . DBManager::RealEscape($this->ReceiverGroup) . "','" . DBManager::RealEscape($this->ReceiverOriginal) . "','" . DBManager::RealEscape($this->Text) . "','" . DBManager::RealEscape($this->Translation) . "','" . DBManager::RealEscape($this->TranslationISO) . "','" . DBManager::RealEscape($this->Received ? 1 : 0) . "','" . DBManager::RealEscape($this->Persistent ? 1 : 0) . "','" . DBManager::RealEscape($this->Repost ? 1 : 0) . "','" . DBManager::RealEscape($this->SenderName) . "','" . DBManager::RealEscape($this->BrowserId) . "','" . DBManager::RealEscape($this->APIObject != null ? json_encode($this->APIObject) : '') . "');");
    }
    ...
    function SaveHistory($type = 0,$iid="",$gid="")
    {
        ...
        if($type!=1)
        {
            ...
            else
            {
                ...
                $ti = time();
                $stmt->bind_param("iiisssssssssssssissssiisssssiiiiis",$ti,$ti,$bi,$id,$bs,$bs,$iid,$gid,$bs,$plain,$html,$bs,$bs,$bs,$bs,$bs,$bi,$bs,$bs,$bs,$bs,$bs,$one,$bs,$bs,$bs,$bs,$bs,$bi,$bi,$bi,$bi,$type,$bs);
...
class Chat extends BaseObject
{
    ...
    function GetHTMLPost($_post,$_translation,$_time,$_senderName,$_senderId)
    {
        ...
        $html = str_replace("<!--color-->","#4989e1",$html);
    ...
    static function CloseChats()
    {
        ...
        while($row = DBManager::FetchArray($result))
        {
            ...
            if($results && $rows = DBManager::FetchArray($results))
            {
                ...
                Chat::SaveToArchive($row["chat_id"],$visitor->VisitorData,$rows["visitor_id"],$lastOp,$rows["request_group"],$visitor->Host,$visitor->IP/*,$chat->Subject*/,(empty(Server::$Configuration->File["gl_sctb"]) && $botchat),$waitingtime,$duration,$startResult,$endResult);
            }
        }
    }
...
class TicketMessage extends Action
{
    ...
    {
        $xml = "<m id=\"".base64_encode($this->Id)."\" s=\"".base64_encode($this->Subject)."\" sid=\"".base64_encode($this->SenderUserId)."\" t=\"".base64_encode($this->Type)."\" c=\"".base64_encode($this->Country)."\" ci=\"".base64_encode($this->ChannelId)."\" ct=\"".base64_encode($this->Created)."\" e=\"".base64_encode($this->Edited)."\" p=\"".base64_encode($this->GetInputData(116,false))."\" cmb=\"".base64_encode(($this->CallMeBack) ? 1 : 0)."\" mt=\"".base64_encode($this->Text)."\" fn=\"".base64_encode($this->GetInputData(111,false))."\" em=\"".base64_encode($this->GetInputData(112,false))."\" ecc=\"".base64_encode($this->EmailCC)."\" ebcc=\"".base64_encode($this->EmailBCC)."\" co=\"".base64_encode($this->GetInputData(113,false))."\" ui=\"".base64_encode($this->SenderUserId)."\" ip=\"".base64_encode($this->IP)."\">\r\n";
        if(is_array($this->Customs))
            foreach($this->Customs as $i => $value)
            {
...
class TicketEmail
{
    ...
    function Destroy($_wipe=false)
    {
        if(!$_wipe)
            ...
            DBManager::Execute(true, "DELETE FROM `" . DB_PREFIX . DATABASE_TICKET_EMAILS . "` WHERE `email_id`='" . DBManager::RealEscape($this->Id) . "' LIMIT 1;");
...
class Ticket extends Action
{
	...
	function GetXML($_full,$_demand=false)
	{
		if($_full)
		{
			...
			$xml = "<val id=\"".base64_encode($this->Id)."\" u=\"".base64_encode($this->LastUpdated)."\" w=\"".base64_encode($wb)."\" sa=\"".base64_encode($this->Salt)."\" del=\"".base64_encode($this->Deleted ? "1" : "0")."\" wl=\"".base64_encode($this->WatchList ? "1" : "0")."\" gr=\"".base64_encode($this->Group)."\" l=\"".base64_encode($this->Language)."\" h=\"".base64_encode($this->GetHash())."\" p=\"".base64_encode($this->Priority)."\" t=\"".base64_encode($this->Channel)."\" s=\"".base64_encode($this->SubChannel)."\">\r\n";
    ...
    function SendAutoresponder($_visitor=null, $_browser=null, $_message=null)
    {
        ...
        for($i=0;$i<count($tsData);$i++)
        {
            ...
            $tsData[$i] = str_replace("%mailtext%",$tText,$tsData[$i]);
            $tsData[$i] = str_replace("%localdate%",date("Y-m-d",$_message->Created),$tsData[$i]);
	...
	function Save($_hash="",$_saveMessages=true)
	{
		...
		if(DBManager::Execute(true, "INSERT IGNORE INTO `" . DB_PREFIX . DATABASE_TICKETS . "` (`id`,`user_id`,`target_group_id`,`hash`,`channel_type`,`sub_channel`,`iso_language`,`channel_id`,`channel_conversation_id`,`channel_unique_id`,`priority`,`salt`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->SenderUserId) . "', '" . DBManager::RealEscape($this->Group) . "', '" . DBManager::RealEscape($_hash) . "', '" . DBManager::RealEscape($this->Channel) . "', '" . DBManager::RealEscape($this->SubChannel) . "', '" . DBManager::RealEscape($this->Language) . "', '" . DBManager::RealEscape($this->ChannelId) . "', '" . DBManager::RealEscape($this->ChannelConversationId) . "', '" . DBManager::RealEscape($this->ChannelUniqueId) . "', " . intval($this->Priority) . ", '" . DBManager::RealEscape($this->Salt) . "');"))
        {
            if($_saveMessages && count($this->Messages) > 0)
            {
                ...
                if($saved === false)
                {
                    Logging::DebugLog($this->Id . ": Double ticket creating, aborting ...");
    ...
    static function Exists($_hash, &$id, &$group, &$language)
    {
        ...
        $result = DBManager::Execute(true, "SELECT `dbt`.`id`,`dbt`.`target_group_id`,`dbt`.`iso_language` FROM `" . DB_PREFIX . DATABASE_TICKETS . "` AS `dbt` WHERE `dbt`.`hash`='" . DBManager::RealEscape($_hash) . "' LIMIT 1;");
        if($result && $row = DBManager::FetchArray($result))
        {
            $id=$row["id"];
...
class ChatRequest extends Action
{
	...
	function Save()
	{
		if(Server::$Operators[$this->SenderSystemId]->IsExternal(Server::$Groups))
...
class ChatArchive
{
    static function GetSearchQuery($_sFor,$_user=false)
    {
        ...
        return "LOWER(`fullname`) LIKE '%".$_sFor."%' OR LOWER(`area_code`) LIKE '%".$_sFor."%' OR LOWER(`plaintext`) LIKE '%".$_sFor."%' OR LOWER(`email`) LIKE '%".$_sFor."%' OR LOWER(`company`) LIKE '%".$_sFor."%' OR LOWER(`phone`) LIKE '%".$_sFor."%' OR LOWER(`chat_id`) LIKE '%".$_sFor."%' OR LOWER(`external_id`) LIKE '%".$_sFor."%' OR LOWER(`question`) LIKE '%".$_sFor."%'";
        //OR LOWER(`html`) LIKE '%".$_sFor."%'
    }
...
class PredefinedMessage
{
    ...
    function __construct()
	{
		...
		else if(func_num_args() == 17)
		{
			...
			$this->ChatInformationOffline = func_get_arg(20);
            ...
            $this->TOSChat = func_get_arg(29);
            $this->TOSTicket = func_get_arg(30);
            */
		}
	}
...
class KnowledgeBase
{
    ...
    static function GetExternalURL($_id="",$_absolute=true,$_version="v1")
    {
        ...
        {
            if($_absolute)
                $url = LIVEZILLA_URL . "knowledgebase.php?entry=show";
            else
...
class KnowledgeBaseEntry
{
    ...
    {
        if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_RESOURCES . "` WHERE `parentid` = '" . DBManager::RealEscape($this->Id) . "' AND `discarded`=0 ORDER BY `title` ASC"))
        {
            while($row = DBManager::FetchArray($result))
            {
    ...
    function Save()
    {
        ...
        {
            if(!$this->IsDiscarded)
                DBManager::Execute(true, "INSERT INTO `" . DB_PREFIX . DATABASE_RESOURCES . "` (`id`,`owner`,`owner_group`,`editor`,`value`,`edited`,`title`,`created`,`type`,`discarded`,`parentid`,`size`,`tags`,`languages`,`kb_public`,`kb_bot`,`kb_ft_search`,`shortcut_word`) VALUES ('" . DBManager::RealEscape(Str::Cut($this->Id,32,false)) . "','" . DBManager::RealEscape($this->OwnerId) . "','" . DBManager::RealEscape($this->GroupId) . "','" . DBManager::RealEscape($this->EditorId) . "','" . DBManager::RealEscape($this->Value) . "'," . intval(time()) . ",'" . DBManager::RealEscape(Str::Cut($this->Title,254,false)) . "'," . intval(time()) . "," . intval($this->Type) . "," . intval($this->IsDiscarded ? 1 : 0) . ",'" . DBManager::RealEscape($this->ParentId) . "','" . DBManager::RealEscape($this->Size) . "','" . DBManager::RealEscape($this->Tags) . "','" . DBManager::RealEscape($this->Languages) . "'," . intval($this->IsPublic ? 1 : 0) . "," . intval($this->AllowBotAccess ? 1 : 0) . "," . intval($this->FulltextSearch ? 1 : 0) . ",'" . DBManager::RealEscape($this->ShortcutWord) . "')");
        }
        else
        {
            $row = DBManager::FetchArray($result);
            DBManager::Execute(true, $result = "UPDATE `" . DB_PREFIX . DATABASE_RESOURCES . "` SET `owner`='" . DBManager::RealEscape($this->OwnerId) . "',`owner_group`='" . DBManager::RealEscape($this->GroupId) . "',`value`='" . DBManager::RealEscape($this->Value) . "',`editor`='" . DBManager::RealEscape($this->EditorId) . "',`tags`='" . DBManager::RealEscape($this->Tags) . "',`title`='" . DBManager::RealEscape(Str::Cut($this->Title,254,false)) . "',`edited`=" . intval(time()) . ",`discarded`='" . intval($this->IsDiscarded ? 1 : 0) . "',`parentid`='" . DBManager::RealEscape($this->ParentId) . "',`size`='" . DBManager::RealEscape($this->Size) . "',`languages`='" . DBManager::RealEscape($this->Languages) . "',`kb_public`=" . intval($this->IsPublic ? 1 : 0) . ",`kb_bot`=" . intval($this->AllowBotAccess ? 1 : 0) . ",`kb_ft_search`=" . intval($this->FulltextSearch ? 1 : 0) . ",`kb_bot`=" . intval($this->AllowBotAccess ? 1 : 0) . ",`shortcut_word`='" . DBManager::RealEscape($this->ShortcutWord) . "' WHERE id='" . DBManager::RealEscape($this->Id) . "' LIMIT 1");
    ...
    {
        if(!empty($_id) && KnowledgeBaseEntry::GetById($_id) == null && strlen($_id) < 32)
        {
