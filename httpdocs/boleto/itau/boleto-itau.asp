...
<!--#include virtual="admin/conexao.asp"-->
<!--#include virtual="include/classe_geral.asp"-->
<%
Conecta()
'=========================================================================================
'Desenvolvimento:
...
if request("id") <> "" then
	session("Cod_Venda_recibo") = request("id")
else
	Response.Write("Est� faltando o c�digo do produto")
end if

session("Cod_venda_recibo") = replace(session("Cod_venda_recibo"),"'","")
session("Cod_venda_recibo") = replace(session("Cod_venda_recibo"),";","")
session("Cod_venda_recibo") = replace(session("Cod_venda_recibo"),"insert","")
session("Cod_venda_recibo") = replace(session("Cod_venda_recibo"),",","")
session("Cod_venda_recibo") = replace(session("Cod_venda_recibo"),"update","")

strsql="select cod_inscricao,nome, endereco, estado, cidade, cep,bairro, TipodePagamento from inscricao where Cod_Inscricao = " & session("Cod_Venda_recibo")
set rs=cn.execute(strsql)

dim x101,x81,x89, x33
'=== Dados Principias para gerar o Boleto
                  'PRE�O EST� FIXO
if(rs("TipodePagamento") = "Boleto") Then
	if isEmpty(request.Form("desconto_boleto")) then
		x101 = formatnumber(1800*85/100,2) ' Valor do Boleto
	else 
		x101 = formatnumber(1800*(100 - int(request.Form("desconto_boleto")))/100,2) ' Valor do Boleto
	end if
	x101 = formatnumber(1,2)
elseif(rs("TipodePagamento") = "Cheque") Then
	x101 = formatnumber(94,2) ' Valor do Boleto	
	'x101 = formatnumber(300,2) ' Valor do Boleto	
end if
x81 = rs("Cod_inscricao") ' Nosso Numero 
x89 = rs("Cod_Inscricao") '  Numero do Documento
x33 = FormatDateTime(dateadd("d",3,now()),2) ' Data Vencimento (Para contra apresentação, deixe o valor em branco, x33 = "" )

Dim x109,x70,x79,x74,x67,x28
'====Dados do Pagador  - Quem paga o Boleto
x109 = rs("nome") ' Nome 
x70 = rs("endereco") ' Endereço 
x79 = rs("bairro") ' Bairro
x74 = rs("cep") ' CEP
x67 = rs("cidade") ' Cidade
x28 = rs("estado") ' Estado 

Dim x136,x56,x52,x49,x83,x64,cli,x10
...
x136= cedente ' Nome do Cedente (Emissor do Boleto)
x56= Endereco_Cadastro
x52= Cidade_Cadastro & " - "& Estado_Cadastro &" - CEP " & Cep_Cadastro'"Cidade - UF - CEP 08060-000"
x49= cpf_cnpj ' CPF ou CNPJ do Cedente
x83 = agencia  ' Numero da Agência sem o Digito (4 Digitos)
x64 = conta ' Numero da Conta sem o digito  (até 5 Digitos) 
cli = rs("Cod_Inscricao") ' Código do cliente (até 5 Digitos) - Utilizado apenas nas carteiras 107, 122, 142, 143, 196 e 198
x10 = carteira ' Numero da Carteira

Dim x80,x171,x130,x97,x73
'===Instruções ao Caixa do Banco (Por exemplo "Não Receber após o vencimento" ou "Cobrar multa de 2 % após o vencimento")
x80	= instrucoes1 ' Campo Instrucoes 1
x171= instrucoes2 ' Campo Instrucoes 2
x130= instrucoes3 ' Campo Instrucoes 3
x97	= instrucoes4 ' Campo Instrucoes 4
x73	= instrucoes5 ' Campo Instrucoes 5

Dim x34,x174,x98,x163,x169,x160
'===dados Padrões  ==============================================
x34=  "DM" ' Especie
x174= "N"' Aceite 
x98=  "" ' Quantidade
...
do Pagador</b></div></TD></tr></tbody></table><table width=666 cellspacing=5 cellpadding=0 border=0><tr><td width=41></TD></tr></table><table width=666 cellspacing=5 cellpadding=0 border=0 align=Default><tr> <%Dim x71,x168%>
<td width=41><IMG SRC="<%=img_logo_loja%>" WIDTH="150" HEIGHT="60"> 
