...
<script type="text/javascript" src="<%="https://"&request.ServerVariables("HTTP_HOST")%>/js/js_carrinho_v7_new.js"></script>
<script>
...
<section class="">
  <div class="container">
    ...
    <div class="row">
	     <%
				Dim tipo_de_pagamento, cod_curso, preco
				...
				sql_curso = "select c.cod_curso, preco from curso c inner join turma t on t.cod_curso = c.cod_curso inner join inscricao i on i.cod_turma = t.cod_turma where cod_inscricao = "& cod_inscricao 				
				set rs_curso = cn.execute(sql_curso)
				
				cod_curso = rs_curso("cod_curso")
				preco = rs_curso("preco")
				'preco = Cdbl(preco) * 85 / 100
		...
		<div class="missed-deals-area" id="pl-showdeal"><!-- &nbsp; --></div> 
                ...
                <div align="left">
                  <select name="parcela" id="parcela" class="form-control input-lg" style="margin-bottom:20px">
                    ...
                    <%for i = 0 to 9
					if i = 0 then%>
                    <option value="<%=(i+1)%>"><%=(i+1)&"x de "&formatcurrency(Cdbl(preco)*85/100)&" À vista"%></option>
                    <%else%>
                    <option value="<%=(i+1)%>"><%=(i+1)&"x de "&formatcurrency(Cdbl(preco)/(i+1))&" Sem juros"%></option>
                    <%end if
					next%>
                  </select>
                </div>
