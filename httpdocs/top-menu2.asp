<%
function verificaTurma(curso, cidade)
	Dim sql_turma,rs_turma
	set rs_turma = cn.execute("SELECT count(t.cod_turma) turmas FROM turma t INNER JOIN studio s ON t.cod_studio = s.cod_studio INNER JOIN uf u ON s.cod_uf = u.cod_uf INNER JOIN cidade ci ON s.cidade = ci.descricao WHERE disponivel = 0 AND t.cod_curso = '"&curso&"' AND ci.url = '"&cidade&"' AND t.status = 'A'")
	verificaTurma = rs_turma("turmas")
end function
%>
          ...
          <li class="dropdown"> <a href="#" class="dropdown-toggle" data-toggle="dropdown">Cursos <b class="caret"></b></a>
            <ul class="dropdown-menu">
              <li><a href="#">Curso de Pilates em Patologias <b class="caret"></b></a>
                <ul>
                  <%
							...
							sql_cidades = "select distinct c.url, c.Descricao from cidade c, studio s, UF u, curso cu, turma t where t.cod_curso = cu.cod_curso and s.cod_uf = u.Cod_uf and u.Cod_uf = c.cod_uf and cu.descricao = 'patologias' and t.status = 'A' order by c.descricao"				
							set rs_cidades = cn.execute(sql_cidades)
							
							do while not rs_cidades.eof
							if verificaTurma(1, rs_cidades("url")) then
							%>
              ...
              <li><a href="#">Curso de Pilates Completo <b class="caret"></b></a>
                <ul>
                  <%							
							sql_cidades = "select distinct c.url, c.Descricao from cidade c, studio s, UF u, curso cu, turma t where t.cod_curso = cu.cod_curso and s.cod_uf = u.Cod_uf and u.Cod_uf = c.cod_uf and cu.descricao = 'completo' and t.status = 'A' order by c.descricao"				
							set rs_cidades = cn.execute(sql_cidades)
							
							do while not rs_cidades.eof
							if verificaTurma(2, rs_cidades("url")) then
							%>
              ...
              <li><a href="#">Curso de Pilates na Gestação <b class="caret"></b></a>
                <ul>
                  <%							
							sql_cidades = "select distinct c.url, c.Descricao from cidade c, studio s, UF u, curso cu, turma t where t.cod_curso = cu.cod_curso and s.cod_uf = u.Cod_uf and u.Cod_uf = c.cod_uf and cu.descricao = 'gestacao' and t.status = 'A' order by c.descricao"				
							set rs_cidades = cn.execute(sql_cidades)
							
							do while not rs_cidades.eof
							if verificaTurma(3, rs_cidades("url")) then
							%>
              ...
              <li><a href="#">Curso de Pilates Suspenso <b class="caret"></b></a>
                <ul>
                  <%							
							sql_cidades = "select distinct c.url, c.Descricao from cidade c, studio s, UF u, curso cu, turma t where t.cod_curso = cu.cod_curso and s.cod_uf = u.Cod_uf and u.Cod_uf = c.cod_uf and cu.descricao = 'suspenso' and t.status = 'A' order by c.descricao"				
							set rs_cidades = cn.execute(sql_cidades)
							
							do while not rs_cidades.eof
							if verificaTurma(4, rs_cidades("url")) then
							%>
