...
if( $conexao ) {
	...
	*/
	$valor_total = $_POST['valor_curso'];
	
	//$valor_total = $_POST['valor_curso'];
	//$valor_total = $_SESSION['valor_curso'];	
	/*
	print($_POST["valor_curso"]);
	print("<br>");
	...
	*/
	...
	else // AUTORIZAÇÃO DIRETA 
	{
		...
		if($Pedido->status == '4' || $Pedido->status == '6'){
			...
			$ch = curl_init("http://www.thepilatesfisiofitness.com.br/compra-concluida.asp?id=$id");
			curl_exec($ch);
		}
		else
		{
