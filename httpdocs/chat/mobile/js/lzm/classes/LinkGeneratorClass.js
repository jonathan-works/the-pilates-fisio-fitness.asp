...
LinkGeneratorClass.prototype.CreateLinkGeneratorHtml = function() {
    ...
    contentHtml += '</span></div></div></div></td><td style="width:60%;height:100%;padding:26px 0 2px 0;">';
...
LinkGeneratorClass.prototype.GetCodeRow = function(id,objB64,name) {
    ...
    html += lzm_inputControls.createButton('delete-element-from-server-btn-' + id, '', 'LinkGeneratorClass.Delete(event,\''+id+'\')', '', '<i class="fa fa-trash" style="color:#808080 !important;"></i>', 'lr',{'margin-right': '0', 'padding-left': '8px', 'padding-right': '8px'}, '', 20, 'b');
    return html + '</td></tr>';
};
...
LinkGeneratorClass.prototype.ShowLinkGeneratorCode = function(){
    ...
    /*UIRenderer.getSettingsProperty(this.GetElementsFromRows(false)[0].m_Settings,'m_UsePassThruStandard').value
...
LinkGeneratorClass.prototype.GetCodeWrapper = function(){
    ...
    html += '<!-- livezilla.net PLACE SOMEWHERE IN BODY -->\r\n<script type="text/javascript" id="' + this.m_CurrentCodeId +'" src="'+DataEngine.getServerUrl('script.php',this.GetSelectedScheme())+'?id=' + this.m_CurrentCodeId +'"></script>\r\n<!-- livezilla.net PLACE SOMEWHERE IN BODY -->';
...
LinkGeneratorClass.prototype.GetCode = function(_preview,_setServer){
    ...
    try
    {
        ...
        if(this.isTypeSelected('inlay-text')){
            ...
            /*
            inlineCode += '<!-- livezilla.net PLACE WHERE YOU WANT TO SHOW TEXT LINK -->\r\n<a href="javascript:void(window.open(\'<!--server_chat--><!--addition-->\',\'\',\'width=<!--width-->,height=<!--height-->,left=0,top=0,resizable=yes,menubar=no,location=no,status=yes,scrollbars=yes\'))" class="<!--class-->" data-text-online="<!--text-on-->" data-text-offline="<!--text-off-->"></a>';
            inlineCode = inlineCode.replace(/<!--class-->/g,'lz_tl');
            inlineCode = inlineCode.replace(/<!--text-on-->/, lz_global_base64_url_encode(elements['inlay-text'].GetProperty('m_TextOnline').value));
            inlineCode = inlineCode.replace(/<!--text-off-->/, lz_global_base64_url_encode(elements['inlay-text'].GetProperty('m_TextOffline').value));
            ...
            */
            ...
            inlineCode += '<!-- livezilla.net PLACE WHERE YOU WANT TO SHOW TEXT LINK -->\r\n<script type="text/javascript" class="lz_text_link" id="'+guid+'" src="<!--server_image--><!--values-->&sid='+guid+'"></script>';
            var values = "?tl=1&srv=" + lz_global_base64_encode(DataEngine.getServerUrl('chat.php',this.GetSelectedScheme()) + this.GetCodeParameters('inlay-text',elements['inlay-text'],'?'));
            ...
            inlineCode = inlineCode.replace(/<!--values-->/g,values);
            inlineCode += '\r\n<!-- livezilla.net PLACE WHERE YOU WANT TO SHOW TEXT LINK -->\r\n\r\n';
        ...
        code = code.replace(/<!--server_chat-->/g,DataEngine.getServerUrl('chat.php',this.GetSelectedScheme())+ '?v=2');
...
LinkGeneratorClass.prototype.GetEyeCatcherObject = function(e){
    ...
    try
    {
        if (e.GetProperty('m_ECUse').value && e.GetProperty('m_ECTypeOptions').value==1)
        {
            ...
            if(!e.GetProperty('m_ECBubbleTitleDefault').value)
            {
                object += 'ec_ht_on:\'' + e.GetProperty('m_ECBubbleTitleOnline').value.replace(/'/g, '\\\'') + '\',';
                object += 'ec_ht_off:\'' + e.GetProperty('m_ECBubbleTitleOffline').value.replace(/'/g, '\\\'') + '\',';
            }

            if(!e.GetProperty('m_ECBubbleSubDefault').value)
            {
                object += 'ec_st_on:\'' + e.GetProperty('m_ECBubbleSubOnline').value + '\',';
                object += 'ec_st_off:\'' + e.GetProperty('m_ECBubbleSubOffline').value + '\',';
            }
...
LinkGeneratorClass.prototype.GetDataPassThruObject = function(element){
    ...
    if(fields.length)
        return '<!-- PASS THRU DATA -->\r<script type="text/javascript">\rvar lz_data = {' + fields + '};\r</script>\r<!-- PASS THRU DATA -->\r\r';
    return '';
};
    ...
    this.m_Settings = [
        {name:'General', title: t('General'), groups: [
            {name: 'Protocol', title: tid('protocol'), controls: [
                {name:'m_ProtocolHTTP', type:'radio', group: 'prot-select', value: true, title: 'HTTP [ <span class="text-red text-bold">' + tid('no_mixed_mode') + '</span> ]', static: true},
        ...
        {name:'Colors', title: t('Colors'), groups: [
            {name: 'Colors', title: t('Colors'), controls: [
                {name:'m_PrimaryColor', type:'color', value:'#4989e1', title: t('Primary Color')},
                {name:'m_SecondaryColor', type:'color', value:'#4078c7', title: t('Secondary Color')},
        ...
        {name:'Eye Catcher', title: t('Eye-Catcher'), groups: [
            ...
            {name: 'ECBubble', class:'m_ECUse m_ECTypeOptionsBubble', title: t('Bubble'), controls: [
                {name:'m_ECBubbleBorderRadius', type:'int', value: '6', title: tidc('border_radius'), left:'single',titleright:'px', top:'single'},
                ...
                {name:'m_ECBubbleBorderColorStart', type:'color', class:'', value:'#4989E1', title: tidc('border_start_color'), left:'single', top:'double'},
                {name:'m_ECBubbleBorderColorEnd', type:'color', class:'', value:'#4078C7', title: tidc('border_end_color'), left:'single', top:'single'},
                ...
                {name:'m_ECBubbleShadowBlur', class:'m_ECBubbleShadowUse', type:'int', value: '5', title: tidc('blur'), left:'double', top:'single'},
                {name:'m_ECBubbleShadowColor', class:'m_ECBubbleShadowUse', type:'color', value:'#666', title: tidc('color'), left:'double', top:'single'},
                ...
                {name:'m_ECBubbleAvatarBGColor', class:'m_ECBubblePersonalize', type:'color', value:'#eee', title: tidc('avatar',' ') + tidc('background',' ')+tidc('color'), left:'double', top:'double'}
            ]},
            ...
            {name: 'ECDimensions', class:'m_ECUse', title: t('Dimensions'), controls: [
                ...
                {name:'m_ECHeight', type:'int', value: '120', title: tidc('height'), left:'single', top:'double',titleright:'px', bottom:'single'}
            ]},
            ...
            {name: 'ECBubbleTitle', class:'m_ECUse m_ECTypeOptionsBubble', title: t('Header Text'), controls: [
                {name:'m_ECBubbleTitleDefault', type:'bool', value: true, title: tid('use_default'), left:'single'},
                {name:'m_ECBubbleTitleOnline', class:'m_ECBubbleTitleDefault', type:'string', value:'', title: tidc('online'), left:'double', top:'single'},
                {name:'m_ECBubbleTitleOffline', class:'m_ECBubbleTitleDefault', type:'string', value:'', title: tidc('offline'), left:'double', top:'single'},
                {name:'m_ECBubbleTitleTextColor', type:'color', class:'', value:'#666', title: tidc('text_color'), left:'single',top:'single'}
            ]},
            {name: 'ECBubbleSubTitle', class:'m_ECUse m_ECTypeOptionsBubble', title: t('Sub Text'), controls: [
                {name:'m_ECBubbleSubDefault', type:'bool', value: true, title: tid('use_default'), left:'single'},
                {name:'m_ECBubbleSubOnline', class:'m_ECBubbleSubDefault', type:'string', value:'', title: tidc('online'), left:'double', top:'single'},
                {name:'m_ECBubbleSubOffline', class:'m_ECBubbleSubDefault', type:'string', value:'', title: tidc('offline'), left:'double', top:'single', bottom:'single'},
                {name:'m_ECBubbleSubTextColor', type:'color', class:'', value:'#777', title: tidc('text_color'), left:'single',top:'single'}
            ]}
        ...
        {name:'APIMode', title: 'API Mode', groups: [
            ...
            {name: 'APIModeCommands', title: 'Javascript Commands', controls: [], custom: true},
            {name: 'APIModeExamples', title: 'Examples', controls: [], custom: true}
    ...
    if(formType == "General")
    {
        ...
        if(this.m_Type == 'overlay-widget-v2')
        {
            ...
            $('.widget-mode').change(function(){
                adjustWidgetMode();
                lzm_commonDialog.createAlertDialog(tid('adjust_margins'), [{id: 'ok', name: tid('ok')},  {id: 'cancel', name: tid('cancel')}]);
                $('#alert-btn-ok').click(function() {
                    lzm_commonDialog.removeAlertDialog();
...
LinkGeneratorElement.prototype.GetCustomForm = function(formType) {
    ...
    else if(formType == "APIModeCommands")
    {
        ...
        contentHtml += '<input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" id="apim-command-show" value="OverlayChatWidgetV2.Show();" spellchecker="false" readonly="readonly" class="code-box">';
        contentHtml += '<br><br><input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" id="apim-command-hide" value="OverlayChatWidgetV2.Hide();" spellchecker="false" readonly="readonly" class="code-box">';
    ...
    {
        contentHtml = '<input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" style="max-width:800px;" value="<span onclick=&quot;OverlayChatWidgetV2.Show();OverlayChatWidgetV2.SetMode(\'chat\');\&quot;>Start Chat</span>" spellchecker="false" readonly="readonly" class="code-box">';
        contentHtml += '<br><br><input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" style="max-width:800px;" value="<img onclick=&quot;OverlayChatWidgetV2.Show();OverlayChatWidgetV2.SetMode(\'ticket\');&quot; src=&quot;./leave_message.png&quot;>" spellchecker="false" readonly="readonly" class="code-box">';
    }
    return contentHtml;
};
...
LinkGeneratorClass.Delete = function(event,id){
