...
class Server
{
    ...
    {
        if(isset($_POST[POST_INTERN_PROCESS_CHAT_ACTION . "_0_vd"]))
            if(in_array($_POST[POST_INTERN_PROCESS_CHAT_ACTION . "_0_vd"],array("AcceptChat","DeclineChat")))
                return true;
        ...
        if($cj_email_out < (time() - 30) && !$_fromApi)
        {
            $action = true;
    ...
    static function SetCronjobTime()
    {
        ...
        DBManager::Execute(true, "REPLACE INTO `" . DB_PREFIX . DATABASE_CONFIG . "` (`key`,`value`) VALUES ('" . DBManager::RealEscape($key) . "','" . intval($time) . "');");
...
class TicketEmail
{
    ...
    function Load()
    {
        $result = DBManager::Execute(true, "SELECT `created`,`sender_cc` FROM `" . DB_PREFIX . DATABASE_TICKET_EMAILS . "` WHERE `email_id`='" . DBManager::RealEscape($this->Id) . "';");
        if($result && $row = DBManager::FetchArray($result))
        {
            $this->Created = $row["created"];
            $this->CC = $row["sender_cc"];
...
class Ticket extends Action
{
	...
	function Save($_hash="",$_saveMessages=true)
	{
		...
		if(DBManager::Execute(true, "INSERT IGNORE INTO `" . DB_PREFIX . DATABASE_TICKETS . "` (`id`,`user_id`,`target_group_id`,`hash`,`channel_type`,`sub_channel`,`iso_language`,`channel_id`,`channel_conversation_id`,`priority`,`salt`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->SenderUserId) . "', '" . DBManager::RealEscape($this->Group) . "', '" . DBManager::RealEscape($_hash) . "', '" . DBManager::RealEscape($this->Channel) . "', '" . DBManager::RealEscape($this->SubChannel) . "', '" . DBManager::RealEscape($this->Language) . "', '" . DBManager::RealEscape($this->ChannelId) . "', '" . DBManager::RealEscape($this->ChannelConversationId) . "', " . intval($this->Priority) . ", '" . DBManager::RealEscape($this->Salt) . "');"))
        {
...
class SocialMediaChannel
{
    ...
    {
        $min = ($_time>0) ? $_time-10 : time()+1;
        DBManager::Execute(true, "UPDATE `" . DB_PREFIX . DATABASE_SOCIAL_MEDIA_CHANNELS . "` SET `last_connect`=" . intval($_time) . " WHERE `last_connect` < ". intval(abs($min)) ." AND `id`='" . DBManager::RealEscape($this->Id) . "'");
        return DBManager::GetAffectedRowCount();
    }
