...
function montaData(data)
	dataV = split(data, ",")
	'Response.Write ubound(dataV) & "-"
	if isDate(dataV(ubound(dataV))) then
		dataFormatada = dataV(ubound(dataV))
		if instr(dataFormatada, "/") > 0 then
			montaData = dataFormatada
		else
			montaData = "Errado"
		end if
		'montaData = dataV(ubound(dataV))
		'Response.Write "A"
	else
		if ubound(dataV) >= 3 then
			ano = replace(dataV(ubound(dataV)), " ", "")
			mes = replace(dataV(ubound(dataV) -1), " ", "")
			dia = replace(dataV(ubound(dataV) - 2), " ", "")
''			Response.Write ano & "/" & mes & "/" & dia
''			response.end 
			if isNumeric(mes) then
				dataFormatada = dia & "/" & mes & "/" & ano
			else			
				dataFormatada = dia & "/" & retornaMes(mes) & "/" & ano			
			end if
			if isDate(dataFormatada) then						
				montaData = dataFormatada
			else
				montaData = "Errado"
			end if	
...
<html lang="pt-br">
<head>
<title>Curso de Pilates - The Pilates Fisio Fitness</title>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta name="description" content="Escola reconhecida e oferece cursos de Pilates Completo e Avançados em Patologias, Suspenso e Gestação.">
<!-- Bootstrap -->
...
<script type="text/javascript" id="b31496a88d56b06ba1090e6bcd527f6c" src="http://www.thepilatesfisiofitness.com.br/chat/script.php?id=b31496a88d56b06ba1090e6bcd527f6c"></script>
<!-- livezilla.net PLACE SOMEWHERE IN BODY -->
            ...
            <div id="myCarousel" class="carousel slide">
               <h1 class="info-head"><span>Não perca os próximos cursos!</span></h1>
                <div class="carousel-inner">
					...
					sql_cursoTurma = "SELECT *, case when c.descricao = 'completo' then 'Pilates Completo' when c.descricao = 'suspenso' then 'Pilates Suspenso' when c.descricao ='patologias' then 'Pilates Patologias' when c.descricao = 'gestacao' then 'Pilates Gestação' end as descricao_curso FROM Turma t INNER JOIN Studio s ON t.cod_studio = s.cod_studio inner join curso c on c.cod_curso = t.cod_curso WHERE status = 'A'"' order by data desc"
					...
					%>
					  ...
					  'Response.Write rs_cursoTurma("data")
					  
					 
					'' Response.Write montaData(rs_cursoTurma("data")) & " = " & now & "<br/>"
					  
					  if not montaData(rs_cursoTurma("data")) = "Errado" then
					  	if cdate(montaData(rs_cursoTurma("data"))) > now then
''					  if teste < 2 then
''						  montaData(rs_cursoTurma("data")) 
''						  teste = teste + 1
''					  end if
					  %>                        
