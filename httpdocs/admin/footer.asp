<!--Basic Scripts-->
    ...
    <script>
		function validar(form){
			extensoesOk= new Array(".png", ".jpg", ".gif"); 
			foto = [];
			
			form.foto1.value ? foto.push(form.foto1.value) : "";
			form.foto2.value ? foto.push(form.foto2.value) : "";
			form.foto3.value ? foto.push(form.foto3.value) : "";
			form.foto4.value ? foto.push(form.foto4.value) : "";  
			
			for (var i = 0; i < foto.length; i++){
				foto[i] = (foto[i].substring(foto[i].lastIndexOf("\\")+1)).toLowerCase();
			}
			for (var i = 0; i < foto.length; i++){
				extensao = (foto[i].substring(foto[i].lastIndexOf("."))).toLowerCase();
				permitida = false;
				for (var j = 0; j < extensoesOk.length; j++){		
					if(extensoesOk[j] == extensao){
						permitida = true;
						break;
					}
				}
				if(!permitida){
					alert("O arquivo " + foto[i] + " é inválido");	
					return false;
				}
			}
			return true;
