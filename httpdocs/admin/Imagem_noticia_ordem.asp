...
Dim codigo,imagem,strsql,rs,ordem
...
<body bgcolor="#F7F4EA" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" scroll="auto">
	...
	<form name="form2" action="upload_noticia_ordem.asp" enctype="multipart/form-data" method="post">
...
if imagem <> "" then
%>
          <img src="/images/noticia/p_<%= imagem %>" border="0"> 
          <%
else
%>
...
if imagem <> "" then
%>
	<input type="button" name="botao" id="botao" onClick="window.open('delete_imagem_ordem.asp','_self')" value="Apagar Foto">
<%
end if
%>
        </div></td>
