...
class GroupBuilder
{
	...
	function __construct($_reqGroup="",$_reqOperator="",$allowCom=true)
	{
		...
		$this->ReqOperator = (!empty($_GET[GET_EXTERN_INTERN_USER_ID])) ? Operator::ReadParams() : $_reqOperator;
	...
	function Generate($_user=null,$_allowBots=false)
	{
		...
		if($counter == 0)
			$this->ErrorHTML = "LiveZillaData.Language.ClientErrorGroups";
	}
...
class ExternalChat
{
    ...
    {
        return Communication::ReadParameter("etc","#4078c7");
    }

    static function ReadBackgroundColor()
    {
        return Communication::ReadParameter("epc","#4989e1");
    }
