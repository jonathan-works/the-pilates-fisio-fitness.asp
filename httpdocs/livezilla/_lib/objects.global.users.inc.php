...
class UserGroup extends BaseUser
{
    ...
    function GetTicketSenderName($_ticket)
    {
        ...
        else if(strpos($this->TicketSenderName,";") !== false)
        {
            ...
            $text = str_replace("%operator_name%",$opName,$text);
            return $text;
        }
...
class Operator extends BaseUser
{
    ...
    function ProcessBotUserAPIReplies($_autoReply,$_apiRESobject,$_visitor)
    {
        ...
        if(isset($_autoReply->Select))
            foreach($_autoReply->Select as $option)
            {
                ...
                {
                    $operatorid = (isset($option->operator)) ? "'" . Encoding::Base64UrlEncode($option->operator) . "'" : "null";
                    $groupid = (isset($option->group)) ? "'" . Encoding::Base64UrlEncode($option->group). "'" : "null";
                    ...
                    if($option->action == "chat")
                    {
                        ...
                        $html .= "OverlayChatWidgetV2.LeaveMessage(true,".$groupid.");";
                }
        ...
        {
            $operatorid = (isset($_apiRESobject->Operator)) ? "'" . Encoding::Base64UrlEncode($_apiRESobject->Operator) . "'" : "null";
            $groupid = (isset($_apiRESobject->Group)) ? "'" . Encoding::Base64UrlEncode($_apiRESobject->Group). "'" : "null";
            ...
            if($_apiRESobject->Action == "chat")
            {
                ...
                $js .= "OverlayChatWidgetV2.LeaveMessage(true,".$groupid.");";
        }
    ...
    function FormatBotAutoReplies($_chat,$_answers,$_alternate=true,$html="",$single="")
    {
        ...
        if(!empty(Server::$Groups[$_chat->DesiredChatGroup]) && Server::$Groups[$_chat->DesiredChatGroup]->IsHumanAvailable())
        {
            ...
            {
                $tth = " " . LocalizationManager::$TranslationStrings["client_or"] . " <a class=\"lz_chat_human\" onclick=\"OverlayChatWidgetV2.ExitBotChat(true,true,3);OverlayChatWidgetV2.InitChat();\">".LocalizationManager::$TranslationStrings["client_talk_to_human"]."</a>.";
                $bind = ", ";
            }
        }

        $lm = (empty(Server::$Configuration->File["gl_dtfbc"])) ? ($bind . " <a class=\"lz_chat_mail\" onclick=\"OverlayChatWidgetV2.LeaveMessage(true);\">" . LocalizationManager::$TranslationStrings["client_leave_a_message"]. "</a>") : "";
        if(count($_answers)==0)
        {
	...
	{
        // AND (`t2`.`user_id`='" . DBManager::RealEscape($this->SystemId) . "' OR `t2`.`user_id`='SYSTEM')
    ...
    static function ReadParams()
    {
        ...
        {
            $userid = Communication::GetParameter(GET_EXTERN_INTERN_USER_ID,"",$c,FILTER_SANITIZE_SPECIAL_CHARS,null,32);
...
class VisitorChat extends VisitorBrowser
{
    ...
    function SetTargetOperatorId($_router,$_user,$_allowBots=false,$_requireBot=false,$_exclude=null,$_closeOnError=true,$_isForward=false,$_botmode=false)
    {
        ...
        else if($_user->Browsers[1]->ChatRequest != null && !$_user->Browsers[1]->ChatRequest->Closed)
        {
            ...
            {
                $routToManualInviteSender = (empty(Server::$Configuration->File["gl_imda"]) && empty($_user->Browsers[1]->ChatRequest->EventActionId));
                if($routToManualInviteSender || $routToAutoInviteSender)
                    $this->SetDesiredChatPartner($_user->Browsers[1]->ChatRequest->SenderSystemId);
                else
                    $this->SetDesiredChatPartner("");
            }
        }
