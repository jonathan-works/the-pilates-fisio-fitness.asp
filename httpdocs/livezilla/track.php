...
if(isset($_GET[GET_TRACK_OUTPUT_TYPE]) && ($_GET[GET_TRACK_OUTPUT_TYPE] == "jscript" || $_GET[GET_TRACK_OUTPUT_TYPE] == "jcrpt"))
{
    ...
    if(!empty($_GET["ovlc"]) && !VisitorMonitoring::$HideElement)
    {
        ...
        VisitorMonitoring::$Response = str_replace("<!--comp_logo-->",Communication::ReadParameter("eh",Server::$Configuration->File["gl_cali"]),VisitorMonitoring::$Response);
...
else
{
	...
	else
	{
        ...
        else
		{
            if(!empty($_GET["ovlc"]))
            {
                VisitorMonitoring::$Response .= "lz_tracking_callback(" . VisitorMonitoring::GetPollFrequency(VisitorMonitoring::$Visitor->IsInChat(false)) . ");";

            }
            else
                VisitorMonitoring::$Response .= "lz_tracking_callback(" . VisitorMonitoring::GetPollFrequency(false) . ");";
		}
	}
}
?>
