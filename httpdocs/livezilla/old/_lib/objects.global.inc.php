...
class TicketEmail
{
    ...
    function Save()
    {
        ...
        else
        {
            $time=SystemTime::GetUniqueMessageTime(DATABASE_TICKET_EMAILS,"edited");
            $errorCode = -1;
            DBManager::Execute(true, "REPLACE INTO `" . DB_PREFIX . DATABASE_TICKET_EMAILS . "` (`email_id`, `mailbox_id`, `sender_email`, `sender_name`,`sender_replyto`,`receiver_email`, `created`, `edited`, `deleted`, `subject`, `body`, `body_html`, `group_id`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->MailboxId) . "', '" . DBManager::RealEscape($this->Email) . "', '" . DBManager::RealEscape($this->Name) . "', '" . DBManager::RealEscape($this->ReplyTo) . "','" . DBManager::RealEscape($this->ReceiverEmail) . "', '" . DBManager::RealEscape($this->Created) . "', '" . DBManager::RealEscape($time) . "', '" . DBManager::RealEscape($this->Deleted ? 1 : 0) . "', '" . DBManager::RealEscape($this->Subject) . "', '" . DBManager::RealEscape($this->Body) . "','" . DBManager::RealEscape($this->BodyHTML) . "', '" . DBManager::RealEscape($this->GroupId) . "');",false,$errorCode);
            if($errorCode == 1366)
                DBManager::Execute(true, "REPLACE INTO `" . DB_PREFIX . DATABASE_TICKET_EMAILS . "` (`email_id`, `mailbox_id`, `sender_email`, `sender_name`,`sender_replyto`,`receiver_email`, `created`, `edited`, `deleted`, `subject`, `body`, `body_html`, `group_id`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->MailboxId) . "', '" . DBManager::RealEscape($this->Email) . "', '" . DBManager::RealEscape($this->Name) . "', '" . DBManager::RealEscape($this->ReplyTo) . "','" . DBManager::RealEscape($this->ReceiverEmail) . "', '" . DBManager::RealEscape($this->Created) . "', '" . DBManager::RealEscape($time) . "', '" . DBManager::RealEscape($this->Deleted ? 1 : 0) . "', '" . DBManager::RealEscape($this->Subject) . "', '" . DBManager::RealEscape(utf8_encode($this->Body)) . "','" . DBManager::RealEscape(utf8_encode($this->BodyHTML)) . "', '" . DBManager::RealEscape($this->GroupId) . "');",false,$errorCode);
        }
...
class KnowledgeBase
{
    ...
    static function GetEntries($_language="")
    {
        ...
        if($result = DBManager::Execute(true,"SELECT * FROM `".DB_PREFIX.DATABASE_RESOURCES."` WHERE `kb_public`=1 AND `type`=0 AND `discarded`=0 ORDER BY `title` ASC"))
        {
            while($row = DBManager::FetchArray($result))
            {
                ...
                if($entry->MatchesLanguage($_language))
                {
                    $entry->LoadChildNodes(true,$_language);
    ...
    {
        $allowed = array("el"=>true,"epc"=>true,"etc"=>true);
...
class KnowledgeBaseEntry
{
    ...
    function LoadChildNodes($_publicOnly=false,$_language="")
    {
