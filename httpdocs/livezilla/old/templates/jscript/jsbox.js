...
{
    lz_livebox_init_center_boxes(true);
}

function lz_livebox_init_center_boxes(_show)
{
    var abs = lz_global_get_window_zoom() <= 1;
    if(lz_center_timeout!=null)
        clearTimeout(lz_center_timeout);
    lz_center_timeout = setTimeout("lz_livebox_center_boxes("+_show+");",100);
}

function lz_livebox_center_boxes(_show)
{
    if(lz_is_mobile)
